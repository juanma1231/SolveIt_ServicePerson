...
public class PersonDTOAssembler implements DTOAssembler<PersonDTO, PersonDomain> {
	...
	public PersonDTO assembleDTO(PersonDomain domain) {
		...
		if(domain != null) {
			dto = new PersonDTO(domain.getId(),domain.getName(),domain.getLastName(),domain.getIdNumber(),null,
					domain.getPhoneNumber(),domain.getMail(),domain.getDescription(),null,domain.getPassword());
		}
		return dto;
	}
